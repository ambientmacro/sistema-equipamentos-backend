...
public class EmailRemetenteService {
    ...
    public List<EmailRemetente> getAll() {
        return emailRemetenteRepository.findAll();
    }

    public EmailRemetente save(EmailRemetente emailRemetente, String actorTipo) {
        if (!"DEVELOPER".equals(normalizeRole(actorTipo))) {
            throw new BusinessException("Apenas DEVELOPER pode cadastrar email remetente");
        }
    ...
    private void validarCredenciais(String email, String senha) {
        ...
        try (Transport transport = session.getTransport("smtp")) {
            ...
            throw new BusinessException("Nao foi possivel validar o remetente. Confira se o email existe e se a senha esta correta.");
        }
    }

    private SmtpConfig resolveSmtpConfig(String email) {
        int arrobaIndex = email == null ? -1 : email.indexOf('@');
        String domain = arrobaIndex >= 0 ? email.substring(arrobaIndex + 1).toLowerCase(Locale.ROOT) : "";
