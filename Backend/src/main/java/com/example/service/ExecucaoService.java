...
public class ExecucaoService {
    ...
    public void arquivarRelatoriosEEncerrarChecklistsPorEstoque(Long estoqueId) {
        ...
        while (true) {
            ...
            // ✅ async (não trava thread)
            ids.forEach(this::processarRelatorioAsync);
    ...
    @Async
    public void processarRelatorioAsync(Long execucaoId) {
    ...
    // ✅ PAGINADO
    @Transactional(readOnly = true)
    @CircuitBreaker(name = "execucaoService", fallbackMethod = "fallback")
    public Page<Execucao> listar(int page, int size) {
        return repository.findAll(criarPageRequest(page, size));
    }

    public Page<Execucao> fallback(int page, int size, Exception ex) {
        return Page.empty();
    }

    // ✅ PERÍODO SEGURO
    @Transactional(readOnly = true)
    public Page<Execucao> buscarPorPeriodo(LocalDateTime inicio, LocalDateTime fim, int page, int size) {
        return repository.findByDataBetween(inicio, fim, criarPageRequest(page, size));
    }

    // ✅ RESUMO (já estava bom)
    @Transactional(readOnly = true)
    ...
    public Page<Execucao> listarPorEstoque(Long estoqueId, int page, int size) {
        return repository.findByEstoqueIdOrderByDataDesc(estoqueId, criarPageRequest(page, size));
    }
    ...
    public List<ExecucaoPainelDTO> listarSemanaAtualPorEstoque(Long estoqueId) {
        ...
        return repository.findPainelSemanaAtualByEstoqueIdAndDataBetween(estoqueId, inicio, fimExclusivo)
                .stream()
    ...
    private void validarChecklistDoDia(Long estoqueId) {
        ...
        if (repository.existsByEstoqueIdAndDataGreaterThanEqualAndDataLessThan(estoqueId, inicio, fimExclusivo)) {
