...
export function useEstoque(estoqueVisao) {
  ...
  const tipoUsuario = String(usuario?.tipoCategoria || "").trim().toUpperCase();
  const isDeveloper = tipoUsuario === "DEVELOPER";
  const canManageCanteiros = isDeveloper || tipoUsuario === "GERENTE" || tipoUsuario === "GERENCIAL";
  const canManageEmpresas = isDeveloper || tipoUsuario === "GERENTE" || tipoUsuario === "GERENCIAL";
  ...
  async function excluirTodosEquipamentos() {
    if (!isDeveloper) {
      window.alert("Apenas DEVELOPER pode excluir todos os equipamentos.");
      return;
    }
