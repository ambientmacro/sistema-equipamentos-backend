...
export default function EquipesCadastradas() {
  ...
  const isGerencia = tipoUsuarioLogado === "GERENCIAL" || tipoUsuarioLogado === "GERENCIA";
  const podeDefinirGerencial = isDeveloper || isGerente;
  const podeCriarTipoEquipe = isDeveloper || isAdmin || isGerencia;
  ...
  const developerTipo = useMemo(
    () => tipos.find((tipo) => normalizarTipo(tipo?.nome) === "DEVELOPER") || null,
    [tipos],
  );
  ...
  const gerencialTipo = useMemo(
    () => tipos.find((tipo) => {
      ...
      return nome === "GERENCIAL" || nome === "GERENCIA";
    }) || null,
    [tipos],
  );
  ...
  const tiposDisponiveis = useMemo(() => {
    return sortByTextKeys(tipos.filter((tipo) => {
      ...
      if (nomeTipo === "DEVELOPER" && !isDeveloper) {
        return false;
      }
      if ((nomeTipo === "GERENCIAL" || nomeTipo === "GERENCIA") && !podeDefinirGerencial) {
        return false;
      }
      return true;
    }), (tipo) => tipo?.nome);
  ...
  const tipoSelecionadoEhGerencial = useMemo(() => {
    const selecionado = tipos.find((tipo) => String(tipo?.id) === String(form.tipoCategoriaId));
    const nome = normalizarTipo(selecionado?.nome);
    return nome === "GERENCIAL" || nome === "GERENCIA";
  }, [tipos, form.tipoCategoriaId]);
  ...
  async function salvarUsuario() {
    ...
    if (!isDeveloper && tipoSelecionadoEhDeveloper) {
      setModalErro("Apenas DEVELOPER pode definir o tipo de usuario como DEVELOPER.");
      return;
    }

    if (!podeDefinirGerencial && tipoSelecionadoEhGerencial) {
      setModalErro("Apenas DEVELOPER e GERENTE podem definir o tipo de usuario como GERENCIAL.");
      return;
    }
    ...
    try {
      ...
      let tipoCadastroId = Number(form.tipoCategoriaId);
        if (form.tipoCategoriaId === NOVO_TIPO_VALUE) {
          ...
          setModalErro("Somente DEVELOPER, ADMIN e GERENCIA podem criar novos tipos de equipe.");
          return;
        }
  ...
  const tiposParaModal = useMemo(() => {
    ...
    const deveManterDeveloperAtual = form.id && developerTipo && String(form.tipoCategoriaId) === String(developerTipo.id);
    const deveManterGerencialAtual = form.id && gerencialTipo && String(form.tipoCategoriaId) === String(gerencialTipo.id);

    if (!deveManterDeveloperAtual && !deveManterGerencialAtual) {
      return tiposDisponiveis;
    }

    const adicionais = [];
    if (deveManterDeveloperAtual && developerTipo) {
      adicionais.push(developerTipo);
    }
  ...
  }, [developerTipo, equipeSelecionada, form.id, form.tipoCategoriaId, gerencialTipo, isDeveloper, podeEditarEquipePreenchida, tipos, tiposDisponiveis]);
      ...
      {modalOpen && (
        ...
        >
          <div className="operacoes-modal">
            ...
            {podeCriarTipoEquipe && (
              <p className="operacoes-modal-helper">
                DEVELOPER, ADMIN e GERENCIA podem criar novos tipos de equipe direto neste cadastro.
              </p>
            )}
            ...
            {(!isDeveloper || !podeDefinirGerencial) && (
              <p className="operacoes-modal-helper">
                {!isDeveloper && !podeDefinirGerencial
                  ? "Apenas usuarios do tipo DEVELOPER podem definir DEVELOPER, e somente DEVELOPER ou GERENTE podem definir GERENCIAL."
                  : !isDeveloper
                    ? "Apenas usuarios do tipo DEVELOPER podem cadastrar ou promover alguem para DEVELOPER."
                    : "Somente DEVELOPER ou GERENTE podem cadastrar ou promover alguem para GERENCIAL."}
              </p>
            )}
