...
export default function RelatoriosGerencial() {
  ...
  const tipoCategoriaUsuario = String(
  ...
  const podeAcessarAnaliseCustos = tipoCategoriaUsuario === "GERENCIAL"
    || tipoCategoriaUsuario === "GERENCIA"
    || tipoCategoriaUsuario === "DEVELOPER";
  ...
  const equipesComLoginSistemaTotal = useMemo(() => {
    ...
    return (Array.isArray(equipes) ? equipes : []).filter((equipe) => {
      const tipoCategoria = String(equipe?.tipoCategoria?.nome || "").trim().toUpperCase();
      const operacional = tipoCategoria !== "ADMIN" && tipoCategoria !== "DEVELOPER";
          ...
          {botoesGraficos.map((botao) => {
            ...
            return (
              <button
                ...
                title={botaoDesabilitado ? "Apenas usuarios GERENCIAL, GERENCIA e DEVELOPER podem acessar Analise de Custos." : ""}
                onClick={() => {
